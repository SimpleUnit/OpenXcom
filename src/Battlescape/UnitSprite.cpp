 ...
 */
UnitSprite::UnitSprite(Surface* dest, const Mod* mod, const SavedBattleGame* save, int frame, bool helmet, int colorPlayer, int colorHostile, int colorNeutral) :
	...
	_helmet(helmet),
	_colorPlayer(colorPlayer), _colorHostile(colorHostile), _colorNeutral(colorNeutral),
...
void UnitSprite::draw(const BattleUnit* unit, int part, int x, int y, int shade, GraphSubset mask, bool drawFacingIndicator)
{
	...
	if (drawFacingIndicator && part == 0)
	{
		...
		{
			tmpSurface->blitNShade(_dest, _x, _y, 0, false, _colorPlayer);
		}
		else
		{
			Surface::blitRaw(_dest, tmpSurface, _x, _y, 0, false, unit->getOriginalFaction() == FACTION_HOSTILE ? _colorHostile : _colorNeutral);
		}
	}
}
