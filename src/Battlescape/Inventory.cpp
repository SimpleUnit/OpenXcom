...
void Inventory::drawItems()
{
	...
	auto attachmentButton = [&](int x, int y, bool enabled)
	{
		RuleInterface *rule = _game->getMod()->getInterface("inventory");
		Uint8 color = rule->getElement("grid")->color;
		Surface surf = Surface(8, 8, x, y);
	...
	if (_selUnit != 0)
	{
		...
		{
			const bool toggleState = invItem->getAttachment() && _attachmentToggles[invItem];
			const Surface *frame = toggleState ? invItem->getAttachment()->getBigSprite(texture, save, _animFrame) : invItem->getBigSprite(texture, save, _animFrame);

			if (invItem == _selItem || !frame)
				continue;

			int x, y, spriteOffX = 0, spriteOffY = 0;
			if (invItem->getSlot()->getType() == INV_SLOT)
			{
				...
				if (toggleState)
				{
					spriteOffX = (invItem->getRules()->getInventoryWidth() * RuleInventory::SLOT_W - invItem->getAttachment()->getRules()->getInventoryWidth() * RuleInventory::SLOT_W) / 2;
					spriteOffY = (invItem->getRules()->getInventoryHeight() * RuleInventory::SLOT_H - invItem->getAttachment()->getRules()->getInventoryHeight() * RuleInventory::SLOT_H) / 2;
				}
			}
			...
			work.executeBlit(frame, _items, x+spriteOffX, y+spriteOffY, 0);
			...
			if (invItem->getSlot()->getType() == INV_HAND)
			{
				...
				if (invItem->getAttachment())
					attachmentButton(
						invItem->getSlot()->getX(),
						invItem->getSlot()->getY() + RuleInventory::HAND_H * RuleInventory::SLOT_H - 8, _attachmentToggles[invItem]);
			}
			else if (invItem->getAttachment())
				attachmentButton(x, y + invItem->getRules()->getInventoryHeight() * RuleInventory::SLOT_H - 7, _attachmentToggles[invItem]);
		...
		{
			const bool toggleState = groundItem->getAttachment() && _attachmentToggles[groundItem];
			const Surface *frame = groundItem->getAttachment() && _attachmentToggles[groundItem] ? groundItem->getAttachment()->getBigSprite(texture, save, _animFrame) : groundItem->getBigSprite(texture, save, _animFrame);
			...
			if (toggleState)
			{
				spriteOffX = (groundItem->getRules()->getInventoryWidth() * RuleInventory::SLOT_W - groundItem->getAttachment()->getRules()->getInventoryWidth() * RuleInventory::SLOT_W) / 2;
				spriteOffY = (groundItem->getRules()->getInventoryHeight() * RuleInventory::SLOT_H - groundItem->getAttachment()->getRules()->getInventoryHeight() * RuleInventory::SLOT_H) / 2;
			}
			...
			{
				const int invHeight = groundItem->getSlotY() + groundItem->getRules()->getInventoryHeight();
				attachmentButton(
					groundItem->getSlot()->getX() + (groundItem->getSlotX() - _groundOffset) * RuleInventory::SLOT_W,
					groundItem->getSlot()->getY() + (invHeight)*RuleInventory::SLOT_H - (invHeight == 3 ? 8 : 7),
					_attachmentToggles[groundItem]);
			}
		}
...
void Inventory::drawSelectedItem()
{
	...
	{
		_selection->clear();
		if (_selItem->getAttachment() && _attachmentToggles[_selItem])
			_selItem->getAttachment()->getRules()->drawHandSprite(_game->getMod()->getSurfaceSet("BIGOBS.PCK"), _selection, _selItem->getAttachment(), _game->getSavedGame()->getSavedBattle(), _animFrame);
		else
 ...
 */
RuleInventory *Inventory::getSlotInPosition(int *x, int *y, bool *corner) const
{
	for (const auto& pair : *_game->getMod()->getInventories())
	{
		if (pair.second->checkSlotInPosition(x, y, corner))
		{
			return pair.second;
		}
	}
 ...
 * @param item Pointer to selected item, or NULL if none.
 */
void Inventory::setMouseOverItem(BattleItem *item, bool attachmentToggle)
{
	_mouseOverItem = (item && !(item->getRules()->isFixed() && item->getRules()->getBattleType() == BT_NONE)) ? item : 0;
	_mouseOverAttachmentButton = attachmentToggle && item;
}

bool Inventory::getMouseOverAttachmentButton()
{
	return _mouseOverAttachmentButton;
}

bool Inventory::getAttachmentToggle(BattleItem *item)
{
	return _attachmentToggles[item];
}
...
void Inventory::mouseOver(Action *action, State *state)
{
	...
	bool attachmentButtonHovered = false;
	RuleInventory *slot = getSlotInPosition(&x, &y, &attachmentButtonHovered);
	if (slot != 0)
	{
		if (slot->getType() == INV_GROUND)
		{
		...
		if (item && attachmentButtonHovered && (item->getSlotX() != x || (slot->getType() != INV_HAND && item->getSlotY() + item->getRules()->getInventoryHeight() - 1 != y)))
			attachmentButtonHovered = false;

		setMouseOverItem(item, attachmentButtonHovered);
	}
	else
	{
		setMouseOverItem(0, false);
	}
...
void Inventory::mouseClick(Action *action, State *state)
{
	if (_game->isLeftClick(action))
	{
		...
		if (_selItem == 0)
		{
			int x = (int)floor(action->getAbsoluteXMouse()) - getX(),
				y = (int)floor(action->getAbsoluteYMouse()) - getY();
			RuleInventory *slot = getSlotInPosition(&x, &y, nullptr);
			if (slot != 0)
			{
				if (slot->getType() == INV_GROUND)
				{
				...
				if (item != 0)
				{
					if (item->getAttachment() && _mouseOverAttachmentButton)
					{
						_attachmentToggles[item] = !_attachmentToggles[item];
					}
		...
		else
		{
			...
			if (slot != 0)
			{
				...
				// Put item in weapon
				else if (item->isWeaponWithAmmo() || (item->getAttachment() && _attachmentToggles[item] && item->getAttachment()->isWeaponWithAmmo()))
				{
					if (item->getAttachment() && _attachmentToggles[item])
						item = item->getAttachment();
					...
					else
					{
						...
						if (_game->isShiftPressed() && (!_tu || tuUnload))
						{
							...
							else
							{
								...
								if ((!item->getAttachHost() && item->getSlot()->getType() != INV_HAND) || (item->getAttachHost() && item->getAttachHost()->getSlot()->getType() != INV_HAND))
								{
									canLoad = false;
								}
							}
						...
						if (canLoad)
						{
							if (!_tu || _selUnit->spendTimeUnits(tuCost))
							{
								...
								if (!quickSwap)
								{
									item->loadClipIntoSlot(slotAmmo, _selItem);
								}
								else
								{
			...
			else
			{
				...
				y = (int)floor(action->getAbsoluteYMouse()) - getY();
				slot = getSlotInPosition(&x, &y, nullptr);
	...
	else if (_game->isMiddleClick(action))
	{
		...
		if (slot != 0)
		{
			...
			{
				if (item->getAttachment() && _attachmentToggles[item])
					Ufopaedia::openArticle(_game, item->getAttachment()->getRules()->getUfopediaType());
				else
...
bool Inventory::unload(bool quickUnload)
{
	...
	BattleItem *unloadedItem = (_selItem->getAttachment() && _attachmentToggles[_selItem]) ? _selItem->getAttachment() : _selItem;


	const BattleType type = unloadedItem->getRules()->getBattleType();
	...
	{
		// Item must be primed
		if (unloadedItem->getFuseTimer() == -1)
		{
			return false;
		}
		if (unloadedItem->getRules()->getFuseTimerType() == BFT_NONE)
		{
			return false;
		}
	...
	else if (weapon)
	{
		...
		{
			if (!unloadedItem->needsAmmoForSlot(slot))
			{
				return false;
			}

			int tu = unloadedItem->getRules()->getTUUnload(slot);
			if (tu == 0 && !_tu)
			{
				return false;
			}

			auto* ammo = unloadedItem->getAmmoForSlot(slot, 0);
			if (ammo)
			{
				toForAmmoUnload = tu;
	...
	BattleActionCost cost { BA_NONE, _selUnit, unloadedItem };
	if (grenade)
	{
...
bool Inventory::isInSearchString(BattleItem *item)
{
	...
	if (articleID && Ufopaedia::isArticleAvailable(_game->getSavedGame(), articleID))
	{
		...
		for (int slot = 0; slot < RuleItem::AmmoSlotMax; ++slot)
		{
			...
			{
				for (const auto& itemAmmoCategoryName : item->getAmmoForSlot(slot, 0)->getRules()->getCategories())
				{
...
bool Inventory::canBeStacked(BattleItem *itemA, BattleItem *itemB)
{
	...
	for (int slot = 0; slot < RuleItem::AmmoSlotMax; ++slot)
	{
		for (int chamberSpot = 0; chamberSpot < RuleItem::AmmoSlotMax; ++chamberSpot)
		{
			auto ammoA = itemA->getAmmoForSlot(slot, chamberSpot);
			auto ammoB = itemB->getAmmoForSlot(slot, chamberSpot);
	...
	if (itemA->getAttachment() && itemB->getAttachment() && !canBeStacked(itemA->getAttachment(), itemB->getAttachment()))
		return false;
