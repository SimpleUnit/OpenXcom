...
void BattlescapeState::drawItem(BattleItem* item, Surface* hand, std::vector<NumberText*> &ammoText, std::vector<NumberText*> &medikitText, NumberText *twoHandedText, bool drawReactionIndicator, bool drawNoReactionIndicator)
{
	...
	if (item)
	{
		...
		twoHandedText->setVisible(rule->isTwoHanded() || rule->isBlockingBothHands());
...
void BattlescapeState::updateUiButton(const BattleUnit *battleUnit)
{
	...
	if (hasPsiWeapon)
	{
		bool canUsePsiWeapon = (psiWeapon->getRules()->getCostsAction(BA_PANIC, battleUnit, psiWeapon).first.Time > 0)
			|| (psiWeapon->getRules()->getCostsAction(BA_USE, battleUnit, psiWeapon).first.Time > 0);
		if (canUsePsiWeapon)
		{
			show(_btnPsi, 1);
		}
