...
void TileEngine::calculateUnitLighting(MapSubset gs)
{
	for (BattleUnit *unit : *_save->getUnits())
	{
		...
		if (unit->getPersonalLight())
		{
			...
			{
				currLight = std::max(currLight, unit->getArmor()->getPersonalLightFriend());
			}
...
bool TileEngine::calculateAnomaliesInFOV(BattleUnit *unit, const Position eventPos, const int eventRadius)
{
	...
	for (auto* anomaly : *_save->getItems())
	{
		...
		int smokeDensityFactor = 100 - Clamp(unit->getVisibilityThroughSmoke(), 0, 100);
		int fireDensityFactor = 100 - Clamp(unit->getVisibilityThroughFire(), 0, 100);

		bool anomalyVisible = anomaly->getDiscovered();
		if (!anomalyVisible)
		{
			int visibilityQuality = visibleDistanceMax * 16 - visibleDistanceVoxels - (densityOfSmoke * smokeDensityFactor + densityOfFire * fireDensityFactor) * getMaxViewDistance() / (3 * 20 * 100);
			ModScript::VisibilityAnomaly::Output arg{visibilityQuality, visibilityQuality, ScriptTag<BattleUnitVisibility>::getNullTag()};
			ModScript::VisibilityAnomaly::Worker worker{unit, anomaly, _save, visibleDistanceVoxels, visibleDistanceMax * 16, densityOfSmoke, densityOfFire, is_dark};
...
TileEngine::ReactionScore TileEngine::determineReactionType(BattleUnit *unit, BattleUnit *target)
{
	...
	BattleItem* disabledLeftAttachment = nullptr;
	BattleItem* disabledRightAttachment = nullptr;
	...
	if (isPlayer)
	{
		...
		if (unit->isLeftHandAttachmentDisabledForReactions())
		{
			if (leftHandItem)
				disabledLeftAttachment = leftHandItem->getAttachHost();
			else if (emptyHandItem)
				disabledLeftAttachment = emptyHandItem->getAttachment();
			else
				nullptr;

		}
		if (unit->isRightHandAttachmentDisabledForReactions())
		{
			if (rightHandItem)
				disabledLeftAttachment = leftHandItem->getAttachHost();
			else if (emptyHandItem)
	...
	{
		if (weapon == disabledLeft || weapon == disabledRight || weapon == disabledLeftAttachment || weapon == disabledRightAttachment)
		{
			...
			while (power_ > 0 && l <= maxRadius)
			{
				if (power_ > 0)
				{
					...
					if (ret.second)
					{
						...
						{
							if (!hitUnit(attack, bi->getUnit(), Position(0, 0, 0), damage, type))
							{
								if (bi->getRules()->getBattleType() == BT_ANOMALY && !bi->getDischarged())
								{
									bi->setDischarged(true);
									Position p = dest->getPosition().toVoxel() + Position(8, 8, -dest->getTerrainLevel());
									_save->getBattleGame()->statePushNext(new ExplosionBState(_save->getBattleGame(), p, BattleActionAttack::GetBeforeShoot(BA_TRIGGER_PROXY_GRENADE, nullptr, bi)));
								}
								if (type->getItemFinalDamage(damage) > bi->getRules()->getArmor())
								{
									toRemove.push_back(bi);
								}
							}
	...
	Collections::removeIf(*unit->getInventory(),
		[&](BattleItem* i)
		{
			if (!i->getRules()->isFixed())
			{
				...
				{
					if (i->getRules()->getCostsAction(BA_UNPRIME, unit, i).first.Time > 0 /* && !i->getRules()->getUnprimeActionName().empty() */ )
					{
			...
			else
			{
				if (deleteFixedItems)
				{
					...
					{
						if (i->needsAmmoForSlot(slot) && i->getAmmoForSlot(slot, 0))
						{
