			...
			|| bu->getOriginalFaction() != FACTION_PLAYER))               // or they're not a soldier
		{
			if (bu->getOriginalFaction() == FACTION_HOSTILE && !bu->isOut())
			{
				...
				else if (bu->getTile())
				{
					...
					{
						bu->instaFalling();
						for (int q = bu->getArmor()->getTotalSize() - 1; q >= 0; --q)
						{
							auto corpse = _save->createItemForTile(bu->getArmor()->getCorpseBattlescape()[q], nullptr);
							corpse->setUnit(bu);
							_save->getTileEngine()->itemDrop(bu->getTile(), corpse, false);
						}
					}
		...
		for (auto* unit : *_save->getUnits())
		{
			...
			for (const auto* layoutItem : *unit->getGeoscapeSoldier()->getEquipmentLayout())
			{
				...
				for (int slot = 0; slot < RuleItem::AmmoSlotMax; ++slot)
				{
					for (int chamberSpot = 0; chamberSpot < RuleItem::ChamberMax; ++chamberSpot)
					{
						if (layoutItem->getAmmoItemForSlot(slot, chamberSpot) != nullptr)
						{
							++toLoad;
						}
						if (layoutItem->getAttachment() && layoutItem->getAttachment()->getAmmoItemForSlot(slot, chamberSpot) != nullptr)
						{
							++toLoad;
						}
					}
					...
					for (auto* ammo : itemList)
					{
						if (ammo->getSlot() == _inventorySlotGround)
						{
							for (int slot = 0; slot < RuleItem::AmmoSlotMax; ++slot)
							{
								for (int chamberSpot = 0; chamberSpot < RuleItem::ChamberMax; ++chamberSpot)
								{
									if (ammo->getRules() == layoutItem->getAmmoItemForSlot(slot, chamberSpot) && matchingAmmoItems[slot][chamberSpot] == nullptr)
									{
									...
									{
										if (ammo->getRules() == layoutItem->getAttachment()->getAmmoItemForSlot(slot, chamberSpot) && matchingAmmoItemsAttachment[slot][chamberSpot] == nullptr)
										{
...
int BattlescapeGenerator::loadMAP(MapBlock *mapblock, int xoff, int yoff, int zoff, RuleTerrain *terrain, int mapDataSetOffset, bool discovered, bool craft, int ufoIndex)
{
	...
	for (auto& extItemDef : *mapblock->getExtendedItems())
	{
		...
		for (auto& extPos : extItemDef.pos)
		{
			...
			auto processAmmoDef = [&](std::vector<std::pair<std::string, int>> theAmmoDef, BattleItem* theItem)
			{
				if (theAmmoDef.empty() || !theItem)
					return;

				for (auto& extAmmoDef : theAmmoDef)
				{
					...
					if (extAmmoRule)
					{
						...
						if (theItem->isWeaponWithAmmo())
						{
							int slotAmmo = theItem->getRules()->getSlotForAmmo(extAmmoRule);
							if (slotAmmo == -1)
							{
								throw Exception("Ammo is not compatible. Weapon: " + theItem->getRules()->getType() + ", ammo: " + extAmmoRule->getType() + ", map block: " + mapblock->getName());
							}
							else
							{
								if (!theItem->loadClipIntoSlot(slotAmmo, newExtAmmoItem))
								{
									throw Exception("Weapon is already loaded. Weapon: " + theItem->getRules()->getType() + ", ammo: " + extAmmoRule->getType() + ", map block: " + mapblock->getName());
								}
							}
						}
						else
			...
			processAmmoDef(extItemDef.ammoDef, newExtItem);
			processAmmoDef(extItemDef.ammoDefAttachment, newExtItem->getAttachment());
		}
	}
...
void BattlescapeGenerator::sendItemsToLimbo()
{
	...
	for (auto* bi : *_craftInventoryTile->getInventory())
	{
		...
		if (bi->getAttachment())
		{
			takeHomeGuaranteed->push_back(bi->getAttachment());
			for (int slot = 0; slot < RuleItem::AmmoSlotMax; ++slot)
			{
				for (int chamberSpot = 0; chamberSpot < RuleItem::ChamberMax; ++chamberSpot)
				{
