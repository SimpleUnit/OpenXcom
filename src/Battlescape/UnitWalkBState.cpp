...
void UnitWalkBState::think()
{
	...
	if (_unit->getStatus() == STATUS_WALKING || _unit->getStatus() == STATUS_FLYING)
	{
		...
		if (_unit->getStatus() == STATUS_STANDING)
		{
			...
			unitSpotted = (!_action.ignoreSpottedEnemies && !_falling && !_action.desperate && _parent->getPanicHandled() &&
				(_numUnitsSpotted != _unit->getUnitsSpottedThisTurn().size() || _unit->hasSpottedNewAnomalies()));
