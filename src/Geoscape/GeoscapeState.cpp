...
void GeoscapeState::timeDisplay()
{
	if (Options::showFundsOnGeoscape)
	{
		if (Options::oxceGeoShowScoreInsteadOfFunds)
		{
			...
			_txtFunds->setText(std::to_string(_game->getSavedGame()->getCurrentScore(_game->getSavedGame()->getMonthsPassed() + 1, _game->getMod())));
		}
		else
		{
			...
			if (xcraft->isDestroyed())
			{
				...
				if (xcraft->getRules()->getMaxUnitsLimit() > 0)
				{
					for (auto soldierIt = xbase->getSoldiers()->begin(); soldierIt != xbase->getSoldiers()->end();)
					{
						...
						{
							_game->getSavedGame()->killSoldier(true, soldier);
							soldierIt = _game->getSavedGame()->finalizeKillSoldier(true, soldier);
						}
						else
						{
							++soldierIt;
