...
AlienDeployment::AlienDeployment(const std::string &type) :
	...
	_scavengeTotalItems(0), _scavengeCompleteText("STR_SUCCESSFUL_RECOVERY")
{
}
...
void AlienDeployment::load(const YAML::YamlNodeReader& node, Mod *mod)
{
	...
	reader.tryRead("scavengeListMain", _scavengeListMain);
	//_scavengeListOptional = node["scavengeListOptional"].as<std::map<std::string, int> >(_scavengeListOptional);
	reader.tryRead("scavengeListOptional", _scavengeListOptional);
	//_scavengeTotalItems = node["scavengeTotalItems"].as<int>(_scavengeTotalItems);
	reader.tryRead("scavengeTotalItems", _scavengeTotalItems);
	reader.tryRead("scavengeCompleteText", _scavengeCompleteText);
	...
	reader.tryRead("scatteredItems", _scatteredItems);
}
