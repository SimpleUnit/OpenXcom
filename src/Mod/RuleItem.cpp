...
void RuleItem::load(const YAML::YamlNodeReader& node, Mod *mod, const ModScript& parsers)
{
	...
	reader.tryRead("multipleDischarges", _multipleDischarges);
	reader.tryRead("proximityRadius", _proximityRadius);
	reader.tryRead("lightRadius", _lightRadius);
	...
	auto loadAmmoConf = [&](int offset, const YAML::YamlNodeReader& n)
	{
		if (n)
		{
			...
			n.tryRead("chamberSize", _chamberSize[offset]);
	...
	reader.tryRead("silenced", _silenced);
