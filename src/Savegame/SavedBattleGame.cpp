...
void SavedBattleGame::load(const YAML::YamlNodeReader& node, Mod *mod, SavedGame* savedGame)
{
	...
	for (auto& keyAndVector : itemKeysAndVectors)
	{
		for (const auto& itemReader : reader[keyAndVector.first].children())
		{
			...
			if (const auto& slotsReader = itemReader["ammoItemSlots"])
				for (size_t slotIndex = 0; slotIndex < RuleItem::AmmoSlotMax * RuleItem::ChamberMax; slotIndex++)
				{
					int itemId = slotsReader[slotIndex].readVal(-1);
					if (itemId > -1 && itemIndex.count(itemId))
						item->setAmmoForSlot(slotIndex % RuleItem::AmmoSlotMax, (slotIndex / RuleItem::AmmoSlotMax), itemIndex.at(itemId));
				}
			else if (const auto& slotReader = itemReader["ammoItem"])
				item->setAmmoForSlot(0, 0, itemIndex.at(slotReader.readVal(0)));

			int attachHostIdx = itemReader["attachHost"].readVal<int>(-1);
			if (attachHostIdx > -1 && itemIndex.count(attachHostIdx))
			{
				BattleItem* attachHost = itemIndex.at(attachHostIdx);
				attachHost->setAttachment(item);
				item->setAttachHost(attachHost);
			}
		}
	}
	...
	reader.tryRead("bta", _beginTurnAnomalies);
...
void SavedBattleGame::save(YAML::YamlNodeWriter writer) const
{
	...
	writer.write("items", _items,
		...
		{ if (!bi->isSpecialWeapon() && !bi->getAttachHost()) bi->save(w.write(), this->getMod()->getScriptGlobal()); });
	...
	writer.write("bta", _beginTurnAnomalies);
