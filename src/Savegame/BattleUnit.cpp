...
BattleUnit::BattleUnit(const Mod *mod, Soldier *soldier, int depth, const RuleStartingCondition* sc) :
	...
	_kneeled(false), _floating(false), _dontReselect(false), _personalLight(false), _defaultPersonalLightDay(false), _defaultPersonalLightNight(false), _aggroPersonalLightDay(false), _aggroPersonalLightNight(false),
...
BattleUnit::BattleUnit(const Mod *mod, Unit *unit, UnitFaction faction, int id, const RuleEnviroEffects* enviro, Armor *armor, StatAdjustment *adjustment, int depth, const RuleStartingCondition* sc) :
	...
	_fallPhase(0), _spottedNewAnomalies(false), _kneeled(false), _floating(false), _dontReselect(false), _personalLight(false),
...
void BattleUnit::load(const YAML::Node &node, const Mod *mod, const ScriptGlobal *shared)
{
	...
	_personalLight = node["personalLight"].as<bool>(_personalLight);
	_defaultPersonalLightDay = node["defPersLightDay"].as<bool>(_defaultPersonalLightDay);
	_defaultPersonalLightNight = node["defPersLightDay"].as<bool>(_defaultPersonalLightNight);
	_aggroPersonalLightDay = node["aggPersLightDay"].as<bool>(_aggroPersonalLightDay);
	_aggroPersonalLightNight = node["aggPersLightDay"].as<bool>(_aggroPersonalLightNight);
	...
	_reactionsDisabledForRightHand = node["reactionsDisabledForRightHandAttachment"].as<bool>(_reactionsDisabledForRightHand);
...
YAML::Node BattleUnit::save(const ScriptGlobal *shared) const
{
	...
	node["personalLight"] = _personalLight;
	if (_defaultPersonalLightDay)
		node["defPersLightDay"] = _defaultPersonalLightDay;
	if (_defaultPersonalLightNight)
		node["defPersLightDay"] = _defaultPersonalLightNight;
	if (_aggroPersonalLightDay)
		node["aggPersLightDay"] = _aggroPersonalLightDay;
	if (_aggroPersonalLightNight)
		node["aggPersLightDay"] = _aggroPersonalLightNight;
	node["fire"] = _fire;
	...
	if (_reactionsDisabledForLeftHandAttachment)
		node["reactionsDisabledForLeftHand"] = _reactionsDisabledForLeftHandAttachment;
	if (_reactionsDisabledForRightHandAttachment)
		node["reactionsDisabledForRightHand"] = _reactionsDisabledForRightHandAttachment;
...
void BattleUnit::setPersonalLight(bool light)
{
...
void BattleUnit::togglePersonalLight()
{
	_personalLight = !_personalLight;
}
 ...
 */
void BattleUnit::toggleRightHandForReactions(bool attachment, bool isCtrl)
{
	if (isCtrl)
	{
		...
		if (attachment)
		{
			_reactionsDisabledForRightHandAttachment = !_reactionsDisabledForRightHandAttachment;
		}
		else
		{
 ...
 */
void BattleUnit::toggleLeftHandForReactions(bool attachment, bool isCtrl)
{
	if (isCtrl)
	{
		...
		if (attachment)
		{
			_reactionsDisabledForLeftHandAttachment = !_reactionsDisabledForLeftHandAttachment;
		}
		else
		{
