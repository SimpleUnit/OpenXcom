...
BattleUnit::BattleUnit(const Mod *mod, Soldier *soldier, int depth, const RuleStartingCondition* sc) :
	...
	_capturable(true), _vip(false), _bannedInNextStage(false), _forceStatus(RETSTAT_OK), _forceRecoverArmor(RECOVERARMOR_KEEP_ARMOR), _skillMenuCheck(false)
{
	...
	if (_geoscapeSoldier)
	{
		...
		{
			const auto skillCost = skill->getCosts(nullptr, nullptr);
			if (_geoscapeSoldier->hasAllRequiredBonusesForSkill(skill)
				&& (skillCost.first.Time > 0 || skillCost.first.Mana > 0)
...
BattleUnit::BattleUnit(const Mod *mod, Unit *unit, UnitFaction faction, int id, const RuleEnviroEffects* enviro, Armor *armor, StatAdjustment *adjustment, int depth, const RuleStartingCondition* sc) :
	...
	_isLeeroyJenkins(false), _summonedPlayerUnit(false), _resummonedFakeCivilian(false), _pickUpWeaponsMoreActively(false), _disableIndicators(false),
	_vip(false), _bannedInNextStage(false), _forceStatus(RETSTAT_OK), _forceRecoverArmor(RECOVERARMOR_KEEP_ARMOR), _skillMenuCheck(false)
{
	if (enviro)
	{
...
void BattleUnit::load(const YAML::YamlNodeReader& node, const Mod *mod, const ScriptGlobal *shared)
{
	...
	reader.tryRead("activeAttachment", _activeAttachment);
	reader.tryRead("preferredHandForReactions", _preferredHandForReactions);
	reader.tryRead("preferAttachmentForReactions", _preferAttachmentForReactions);
	...
	reader.tryRead("reactionsDisabledForLeftHandAttachment", _reactionsDisabledForLeftHandAttachment);
	reader.tryRead("reactionsDisabledForRightHandAttachment", _reactionsDisabledForRightHandAttachment);
...
void BattleUnit::save(YAML::YamlNodeWriter writer, const ScriptGlobal *shared) const
{
	...
	if (_activeAttachment)
		writer.write("activeAttachment", _activeAttachment);
	if (!_preferredHandForReactions.empty())
	{
		...
		if (_preferAttachmentForReactions)
			writer.write("preferAttachmentForReactions", _preferAttachmentForReactions);
	}
	...
	if (_reactionsDisabledForLeftHandAttachment)
		writer.write("reactionsDisabledForLeftHandAttachment", _reactionsDisabledForLeftHandAttachment);
	if (_reactionsDisabledForRightHandAttachment)
		writer.write("reactionsDisabledForRightHandAttachment", _reactionsDisabledForRightHandAttachment);
	...
	if (save->getBattleState())
	{
		...
		if (attack.attacker)
		{
			...
			if (getFaction() != attack.attacker->getFaction() &&
				...
				attack.damage_item != nullptr && !attack.weapon_item->getRules()->getSilenced() &&
